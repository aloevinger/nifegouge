 ...
 * const randomEP = epDivs[Math.floor(Math.random() * epDivs.length)]; // Random EP
 */

export const getEPDivs = ({ epsData, handleEPsChange, getInputClass, styles }) => {
  const { epSectionStyle, epHeaderStyle, epStepStyle, epInputStyle, decisionPointStyle, subStepStyle } = styles;
...
// Export EP names for easy reference
export const EP_NAMES = [
...
// Helper function to get a random EP index
export const getRandomEPIndex = () => Math.floor(Math.random() * EP_NAMES.length);
