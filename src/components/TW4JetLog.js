...
function TW4JetLog() {
  ...
  const [clncFields, setClncFields] = useState({ ttc: '', fuel: '', dist: '', deltaT: '', oat: '', tasClimb: '', lbsPhClimb: '', tasCruise: '', lbsPhCruise: '', ias: '', climbDir: '', climbVel: '', cruiseDir: '', cruiseVel: '' });
    ...
    fetch('/Airport Info.csv')
      ...
      .then(text => {
        ...
        setAirports(rows.map(row => {
          const p = row.split(',');
          return { airport: p[0], elev: p[1], clncDel: p[3], gndCont: p[4], tower: p[5], apcCont: p[6] };
        }));
      });
  }, []);
  ...
  useEffect(() => {
    ...
    setInputValues(prev => ({
      ...prev,
      'r0c4': params.sttoTime,
      'r0c6': params.sttoFuel,
      'r0c7': String(startF - sttoF),
    }));
  }, [params.startFuel, params.sttoFuel, params.sttoTime]);
  ...
  const autoFillCruise = (oat, alt) => {
    ...
    cruiseData.forEach(r => {
      if (!altGroups[r.alt]) altGroups[r.alt] = [];
      altGroups[r.alt].push(r);
    });
    const altList = Object.keys(altGroups).map(Number).sort((a, b) => a - b);
  ...
  const renderFreqCell = (key, label, tdProps = {}) => {
    ...
    const handleChange = (e) => {
      setInfoField(key, e.target.value);
    };
    return (
      <td {...tdProps}>
        <span className="info-label">{label}</span>
        <div style={{position: 'relative'}}>
          <input type="text" value={main} onChange={handleChange} />
  ...
  const renderCell = (cellId, rowSpan, colSpan) => {
    if (splitCells[cellId]) {
      ...
      return (
        <td id={cellId} rowSpan={rowSpan || undefined} colSpan={colSpan || undefined} style={{padding: 0, border: '1px solid black', verticalAlign: 'middle'}}>
  ...
  const computeFuelPlan = () => {
    ...
    const item1 = totalTopFuel - sttoFuelParam - (totalApproachCount * approachFuelParam);
    const item2 = totalAltFuel;
    const item3 = approachFuelParam * (1 + totalApproachCount);
  ...
  const renderTotalRow = (tTop, tBot, distIds, eteIds, fuelIds) => (
    <React.Fragment>
      <tr>
  ...
  const generateFlightPlan = async () => {
    try {
      ...
      const tas = Math.round(parseFloat(clncFields.tasCruise) || 0);
      const speedStr = 'N0' + String(tas).padStart(3, '0');

      // Level: A090 for 9,000 ft MSL, FL220 for 22,000 ft MSL
      const levelStr = selectedAlt >= 18000
      ...
      const totalETE = mainRowsArr.reduce((sum, row) => {
        const v = parseFloat((inputValues[`r${row}c4`] || '').match(/-?\d+(\.\d+)?/)?.[0]);
      ...
      for (let pairIdx = 0; pairIdx < mainRowCount; pairIdx++) {
        ...
        if (badge === 'H' && botVal) {
          ...
          dleEntries.push(`DLE/${botVal}${String(Math.floor(hMins / 60)).padStart(2, '0')}${String(hMins % 60).padStart(2, '0')}`);
  ...
  return (
    <div className="jetlog-container" onKeyDown={handleKeyNav}>
      ...
      <>
      <div style={{fontSize: '0.68em', color: '#888', textAlign: 'center', marginBottom: '2px'}}>
        Check that the Jet Log Parameters are set as you like. Input winds, ΔT, altitude, and OAT; MC and Dist for legs; mark approaches and holds as appropriate; and click solve.
        Input total distance into the DIST cell, the solver will split it into climb and cruise dist.
      </div>
          ...
          <tr>
            <td style={{position: 'relative'}}>
              ...
              <div style={{display: 'flex', alignItems: 'center'}}>
                <input type="text" value={infoFields.depElev} onChange={e => { const v = e.target.value; const match = airports.find(ap => ap.airport.toUpperCase() === v.trim().toUpperCase()); if (match) { selectAirport(match, 'dep'); } else { setInfoField('depElev', v); if (clncFields.deltaT) autoFillTTC(clncFields.deltaT, selectedAlt, v); } }} style={{flex: 1}} />
      ...
      <table className="jetlog-info-table">
        ...
        <tbody>
          <tr>
            ...
            <td colSpan={10}><span className="info-label">ROUTE</span><input type="text" /></td>
            <td colSpan={10}><span className="info-label">ALTITUDE</span><input type="text" placeholder="Hundreds of ft" /></td>
            <td colSpan={5}><span className="info-label">FUEL</span><div style={{textAlign: 'center', fontSize: '0.75em', paddingTop: '1px'}}>{computeFuelDisplay()}</div></td>
            <td colSpan={5}><span className="info-label">TIME</span><div style={{textAlign: 'center', fontSize: '0.75em', paddingTop: '1px'}}>{computeTimeDisplay()}</div></td>
          </tr>
          <tr>
      ...
      <table className="jetlog-table" style={{marginTop: '0'}}>
        ...
        <tbody>
          {(() => {
            ...
            const fp = computeFuelPlan();
            return fp.map(([leftVal, rightVal], i) => (
              <tr key={i}>
        ...
        >
          ...
          >
            ...
            {[
              ...
              {label: 'Hold Time (min)',         key: 'holdTime'},
